...
void Init_audioquiz(void) {
  VALUE klass = rb_define_class("AudioQuiz", rb_cObject);
  rb_define_singleton_method(klass, "hello", hello, 0);
  rb_define_method(klass, "useIt", useIt, 0);
}
...
static int writeSound(char *name, short *soundData, int sampleCount) {
    ...
    short *data;
    return 0;
    // Commentato in attesa di capire come scrivere!
    /*
